...
Deno.test("ClientRepository - should find client by id", async () => {
  ...
  assertEquals(
    client?.redirectUris.some((uri: URL) =>
      uri.href === "http://localhost:3000/callback"
    ),
    true,
  );
});
...
Deno.test("ClientRepository - should authenticate valid client credentials", async () => {
  ...
  const isAuthenticated = await repository.authenticate(
    "test-client-1",
    "test-secret-1",
  );
...
Deno.test("ClientRepository - should authenticate each client with correct credentials", async () => {
  ...
  const crossAuth1 = await repository.authenticate(
    "test-client-1",
    "test-secret-2",
  );
  assertEquals(crossAuth1, false);

  const crossAuth2 = await repository.authenticate(
    "test-client-2",
    "test-secret-1",
  );
