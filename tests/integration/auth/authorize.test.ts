...
Deno.test("Authorize endpoint - should validate required parameters", async () => {
  ...
  const request = new Request(`http://localhost:8000/auth/authorize?${params}`, {
    method: "GET",
    headers: {
...
Deno.test("Authorize endpoint - should redirect to signin without authentication", async () => {
  const params = new URLSearchParams({
    response_type: "code",
    client_id: "test-client-id",
    redirect_uri: "http://localhost:3000/callback",
...
Deno.test("Authorize endpoint - should reject invalid scope", async () => {
  const params = new URLSearchParams({
    response_type: "code",
    client_id: "test-client-id",
    redirect_uri: "http://localhost:3000/callback",
...
Deno.test("Authorize endpoint - should reject invalid response_type", async () => {
  const params = new URLSearchParams({
